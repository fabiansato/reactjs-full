<div align="center">
  <h1> 30 Modulos Of React:Component Life Cycles</h1>
...
[<< Modulo 13](../13_Modulo_Controlled_Versus_Uncontrolled_Input/13_uncontrolled_input.md) | [Modulo 15 >>](../15_Third_Party_Packages/15_third_party_packages.md)

![30 Modulos of React banner](../images/30_Modulos_of_react_banner_Modulo_14.jpg)
...
NowaModulos we write class based-component without a constructor and we can write the state also outside the constructor. In older version React we the state used be always inside the constructor.
...
For instance, if we want to stop doing challenge after 30 Modulos we can increment the Modulo from 1 to 30 and we can block the application at Modulo 30.
Look the example.
...
class App extends Component {
  ...
  shouldComponentUpdate(nextProps, nextState) {
    console.log(nextProps, nextState)
    console.log(nextState.Modulo)
    if (nextState.Modulo > 31) {
      return false
    } else {
      return true
    }
  }
  // the doChallenge increment the Modulo by one
  doChallenge = () => {
    this.setState({
  ...
  render() {
    return (
      <div className='App'>
        ...
        <p>Challenge: Modulo {this.state.Modulo}</p>
